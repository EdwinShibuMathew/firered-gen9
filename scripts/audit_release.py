...
def main() -> int:
    ...
    checks = {"build_lock_json": bool(lock.get("upstreams")) and bool(lock.get("artifacts")), "required_docs_and_ledgers": all((ROOT / p).exists() for p in required), "release_rom_policy": not any(Path(p).suffix.lower() in forbidden for p in tracked)}
    for name, ok in checks.items(): print(f"{'PASS' if ok else 'FAIL'} {name}")
    print("PENDING owner-led manual test matrix, public asset review, and current BPS round-trip validation")
